...
def view_player(identifier):
    ...
    if 'AbsorptionAmount' in nbt_data:
        print(f"Absorption: {nbt_data['AbsorptionAmount']:.1f}")
    if 'foodLevel' in nbt_data:
        food = nbt_data['foodLevel']
        saturation = nbt_data.get('foodSaturationLevel', 0)
    ...
    if 'Rotation' in nbt_data:
        rot = nbt_data['Rotation']
...
def edit_player(identifier, field, value):
    ...
    # Special command: give item
    if field == 'give':
        give_item(identifier, value, nbt_data, dat_file)
        return
    ...
    # Write back to file
    save_player_data(nbt_data, dat_file)
    
    print(f"\n✓ Updated {field}: {old_value} → {new_value}")


def give_item(identifier, item_spec, nbt_data=None, dat_file=None):
    """Give an item to a player
    
    Format: item_id[:count][:nbt_json]
    Examples:
        minecraft:diamond_sword
        minecraft:bread:16
        minecraft:stick:1:{"components":{"minecraft:custom_name":{"text":"Magic Stick"}}}
    """
    if nbt_data is None:
        result = view_player(identifier)
        if not result:
    ...
    parts = item_spec.split(':', 2)
    if len(parts) < 2:
        print("Error: Item must be in format 'namespace:item_id' (e.g., minecraft:diamond)")
        return
    ...
    # Check for count and NBT data
    remaining = ':'.join(parts[2:]) if len(parts) > 2 else ''
    if remaining:
        # Try to parse as count:nbt or just nbt
        if remaining[0] == '{':
            # It's NBT JSON
            try:
                components = json.loads(remaining).get('components', {})
            except json.JSONDecodeError as e:
                print(f"Error parsing NBT JSON: {e}")
                return
        else:
            # Try to split count and NBT
            nbt_parts = remaining.split(':', 1)
            try:
                count = int(nbt_parts[0])
            except ValueError:
                print(f"Error: Invalid count '{nbt_parts[0]}'")
                return
            
            if len(nbt_parts) > 1 and nbt_parts[1]:
                try:
                    components = json.loads(nbt_parts[1]).get('components', {})
                except json.JSONDecodeError as e:
                    print(f"Error parsing NBT JSON: {e}")
                    return
    ...
    empty_slot = None
    for slot in range(36):  # 0-35 are main inventory slots
    ...
    nbt_data['Inventory'] = inventory
    
    # Write back to file
    save_player_data(nbt_data, dat_file)
    
    print(f"\n✓ Gave {count}x {item_id} to player (slot {empty_slot})")
    if components:
        print(f"  With components: {json.dumps(components, indent=2)}")
...
Examples:
  ...
  # Give items (simple):
  %(prog)s give Steve minecraft:diamond_sword
  %(prog)s give Steve minecraft:bread:16
  
  # Give items with NBT (use JSON):
  %(prog)s give Steve 'minecraft:stick:1:{"components":{"minecraft:custom_name":{"text":"Magic Stick","italic":false}}}'
  %(prog)s give Steve 'minecraft:diamond_sword:1:{"components":{"minecraft:enchantments":{"levels":{"minecraft:sharpness":5}}}}'
        '''
    )
    ...
    # Give command
    give_parser = subparsers.add_parser('give', help='Give item to player')
    give_parser.add_argument('player', help='Player name or UUID')
    give_parser.add_argument('item', help='Item specification (see examples)')
    ...
    if args.command == 'list':
        ...
        give_item(args.player, args.item)
